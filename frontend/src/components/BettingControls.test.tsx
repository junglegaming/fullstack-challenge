...
describe("BettingControls", () => {
  it("shows betting countdown during betting phase", () => {
    render(
      <BettingControls
        cashingOut={false}
        onCashOut={vi.fn()}
    ...
    expect(screen.getByTestId("betting-countdown")).toHaveTextContent(
      /Betting closes in/,
    );
  });
  ...
  it("enables cash out with potential payout when user has a placed bet", () => {
    render(
      <BettingControls
        cashingOut={false}
    ...
    expect(
      screen.getByRole("button", { name: "Cash Out ($ 25.00)" }),
    ).toBeEnabled();
  });
});
